...
export const Top = () => {
  const { setMore } = useCommentsStore();
  const { added_by: addedBy, date, _id } = useContext(CommentContext);
  ...
  return (
    <section className="mb-[2px] grid h-[20px] w-full max-w-full grid-cols-[calc(90%_-_20px)_10%] gap-[20px]">
      <div
        onClick={onNavigate}
        className="flex h-full w-full items-start justify-start gap-[5px]"
      >
        <div className="max-w-[45%] overflow-hidden text-ellipsis whitespace-nowrap text-text">
          <span className="w-full cursor-pointer text-ellipsis text-sm font-bold">
            {addedBy.name}
          </span>
        </div>
        <div className="max-w-[45%] overflow-hidden text-ellipsis whitespace-nowrap text-decorateorange/70">
